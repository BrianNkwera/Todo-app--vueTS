...
<template>
  ...
  <div v-else>
    ...
    <div v-else>
      <ToDoItem class="mb-3" :key="todo.id" v-for="todo in props.todos" :id="todo.id" :title="todo.title"
        :completed="todo.completed" :description="todo.description" :created="todo.created"
        :lastUpdated="todo.lastUpdated" @checkedCompleted="checkedCompleted(todo)"
        @deleteTodo="openConfirmationModal(todo.id)" @editTodo="editTodo(todo)" />
    </div>
  </div>
